*==============================================================================*
*  xtpraisk simulation study -- Do-file 3: AR(3) primary + misspecification
*
*  Estimators:
*    Primary:  xtpraisk lag(3)  vs  xtscc lag(3)   [DGP=AR(3), fit=AR(3)]
*    Misspec:  aligned pairs at each fitted lag order:
*              lag(3): xtpraisk lag(3) vs xtscc lag(3)  [correct]
*              lag(2): xtpraisk lag(2) vs xtscc lag(2)  [underspecified by 1]
*              lag(1): xtpraisk lag(1) vs xtscc lag(1)  [underspecified by 2]
*
*  xtpraisk and xtscc always use the same lag -- complete alignment.
*
*  Primary design : 3 scenarios x 3 N x 6 T x 4 b1 = 216 conditions
*  Misspec design : 3 scenarios x 3 N x 6 T x 2 b1 = 108 conditions
*                   6 methods per condition (PW/DK each at lag 3,2,1)
*  2,000 replications each
*
*  DGP (Beck & Katz 1995; Hoechle 2007):
*    y_it = b0 + b1*x_it + u_it
*    u_it = rho1*u_i,t-1 + rho2*u_i,t-2 + rho3*u_i,t-3 + eps_it
*    x_it ~ iid N(0,1) redrawn each replication
*    First 3 obs per panel initialized via 200-period burn-in
*
*  Rho structures follow Linden (2026b):
*    S1: rho = (0.4,  0.2,  0.1)  mild positive
*    S2: rho = (0.7, -0.3,  0.15) oscillatory
*    S3: rho = (0.6,  0.25, 0.1)  high persistent
*
*  Output column "reject" = proportion rejecting H0: b1=0
*    => type I error when b1_true==0, power when b1_true!=0
*
*  Output: xtpraisk_sim_ar3.dta  and  xtpraisk_sim_ar3_misspec.dta
*==============================================================================*
...
*==============================================================================*
*  PRIMARY: DGP=AR(3), fit=AR(3) -- xtpraisk and xtscc both use lag(3)
*==============================================================================*
